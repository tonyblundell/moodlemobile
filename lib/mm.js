...
var MM = {
    ...
    connectionWarningShowing: false,
    ...
    init: function(config) {
        ...
        // Set up the connection monitor which checks every 5 seconds to see
        // whether the device has internet access.
        var connectionMonitor = setInterval(function(){
            // If the device isn't connected...
            if (!MM.deviceConnected()) {
                // ... and the warning message hasn't already been shown
                if (MM.connectionWarningShowing === false) {
                    // Display the warning message
                    MM.popMessage(MM.lang.s('connectionwarningmessage'), {});
                }
                // And set that we're now displaying the message.
                MM.connectionWarningShowing = true;
            } else {
                // If the device is connected then the connection warning
                // can be reshown if necessary.
                MM.connectionWarningShowing = false;
            }
        }, 5000);
    },
    ...
    loadLayout: function() {
        ...
        var navigation_plugin = MM.config.menu_type;
        MM.navigation = MM.plugins[navigation_plugin];
        if (MM.navigation === undefined) {
            MM.log('Navigation plugin ' + navigation_plugin + ' did not exist');
        }
    ...
    loadCourses: function(courses) {
        // Load the main menu template on the left, passing in the above values.
        MM.navigation.render(MM.config.menu_location);
