...
MM.rdebugger = {
    ...
    getMessages: function() {
        ...
        var baseURL = MM.config.current_site.siteurl + "/local/mmrdebugger/messages.php";
        ...
        $.ajax({
            ...
            success: function(messages) {
                ...
                $.each(messages, function(index, message) {
                    ...
                    if (message.type == "command" && message.text) {
                        ...
                        try {
                            ...
                            /// Maybe a little additional security will be interesting. Caja or jssandbox are good candidates.
                        ...
                        MM.rdebugger.takeScreenshot(baseURL, message.id);
                ...
                $.each(responsesQ, function(index, response) {
                ...
                if (MM.rdebugger.streaming !== false) {
                    MM.rdebugger.streamPage(baseURL, MM.rdebugger.streaming);
                }
            },
            error: function(e) {
                MM.log("Rdebug: Error retrieving url: " + baseURL);
    ...
    takeScreenshot: function(baseURL, messageId) {
        if (typeof(html2canvas) == "undefined") {
            $.getScript('externallib/html2canvas.js', function(d) {
                html2canvas(document.body, {
                    onrendered: function(canvas) {
                        MM.rdebugger.sendScreenshot(baseURL, messageId, canvas.toDataURL("image/png"));
                        // Free memory
                        canvas = null;
                    }
                  });
    ...
    sendScreenshot: function(baseURL, messageId, screenshot) {
        var data = {
            "token": MM.config.current_token,
            "action": "reply_message",
            "messageid": messageId,
    ...
    streamPage: function(baseURL, messageId) {
        var css= [];
